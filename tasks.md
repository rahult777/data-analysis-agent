...
- [ ] **HIGH PRIORITY — Execute the Cleaner's own (model-authored) decisions by structured operation id instead of keyword matching.** The keyword router runs a different operation than the report claims, or none: live evidence in F3 C3 and F3b C4 (region normalisation ran nothing; return_flag mode-filled the 15 values the model said it would leave alone; "no duplicates, no action" removed 15 rows), plus the Step 8 flag-and-include → mean-fill probe. Changes the model's CleaningReport contract, so it is prompt-facing and needs its own evaluation and live validation; see errors.md 2026-09-25.
...
- [ ] Derive compute_data_quality_score's outlier penalty from structured facts instead of decision-text keywords; see errors.md 2026-09-25
...
- [ ] Enforce the Analyzer's 30-pair correlation floor in Python — after the Analyzer response, force `confidence_level = "Cannot Determine"` on every strong pair with fewer than 30 complete pairs (3 of 3 affected stored analyses say Low), plus an Explainer self-check of numeric comparisons against the stated floors; see errors.md 2026-09-21 ("Analyzer states 'n=15 ... above the 30-pair reliability floor'")
...
- [ ] Handle domain-pause options whose ids are not "confirm"/"correct" — /resume accepts them but build_domain_resolution raises (map by the option's `action`, or reject at /resume); see errors.md 2026-09-22
...
- [ ] Compute Cleaner pause counts (and the missingness backstop's threshold check) on the frame after duplicate removal and earlier answers, not the raw upload; see errors.md 2026-09-25 ("Counts in a Cleaner pause describe the raw upload")
- [ ] Make the two remaining inline IQR-bound computations (detect_interactions; execute_cleaning_operations' OUTLIER FLAG branch) call `_iqr_bounds` / `_iqr_outlier_mask` — behavior-preserving cleanup with a pure-move check; see errors.md 2026-09-25 ("The IQR outlier bounds are computed in four places")
...
- [ ] Pause-state UI — frontend half: pause question components on top of GET /status `pause_data`, the resume form per the decisions.md 2026-09-22 contract, read-only for visitors via `isOwner` (only the owner answers) — UNBLOCKED 2026-09-25: Build F3 is fully live-validated
