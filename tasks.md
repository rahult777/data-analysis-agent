...
- [x] Wire DM Sans into Tailwind (2026-09-22, commit pending review) — `tailwind.config.ts` maps `fontFamily.sans` to `var(--font-sans)` plus v3's default stack as the generic fallback; `font-sans`, the `html` rule and Preflight now resolve to DM Sans (body text), completing decisions.md 2026-05-16. Instrument Serif needed no change (already applied by inline `var(--font-display)`). Verified by computed styles before/after on the upload page and 1a96db4a at 320px/1280px, collapsed and expanded (serif and mono byte-identical, overflow 0); tsc, ESLint and build clean; backend 282/16 unchanged. See decisions.md 2026-09-22 and errors.md 2026-09-21 (status update) / 2026-09-22 (chart font — open question).
...
- [~] Build F3: the Cleaner consumes pause answers across multiple pauses — answers accumulate in state, a repeat pause raises, Python checks every pause before it is shown, executes each user choice by option id (report == what ran, attributed to the user), backstops a skipped >30% missing-value pause, and adds an honest 4th option (`preserve_missingness`); cleaner:74 and both "unknown" premises fixed. Built and tested (391 passed / 16 skipped; 25 of 25 mutations caught; two Code Review passes). PARTIALLY live-validated 2026-09-25 on messy_data.csv ($0.4109 of $1.25): accumulation, the repeat guard, execution by option id and the missing-value backstop passed. BLOCKED on the outlier backstop: the model skipped the mandatory financial-outlier pause, which F3 does not backstop. See decisions.md 2026-09-25 (Build F3).
- [ ] Outlier-pause backstop in the Cleaner (narrow: exact canonical medical/financial domain, IQR outliers from F3's raw-upload mask) — evaluation next; completes F3's live validation; see errors.md 2026-09-25
- [ ] Compute `duplicate_row_count` in Python and overwrite the Profiler's copy (it reported 0 against 15 on messy_data.csv); see errors.md 2026-09-25
- [ ] Frontend pause UI — unblocked once Build F3 is live-validated (visitors see the pause read-only via isOwner; only the owner answers)
- [ ] Execute the Cleaner's own (model-authored) decisions by structured operation id instead of keyword matching — HIGH PRIORITY, its own evaluation; see errors.md 2026-09-25
- [ ] Pause-state UI — frontend half (pause question components on top of GET /status `pause_data`, the resume form per the decisions.md 2026-09-22 contract, read-only for visitors via `isOwner`) — blocked on the item above
