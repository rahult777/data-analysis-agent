...
- [~] Build F1: consume the domain-pause answer — code, prompt and 30 tests done (uncommitted; 312 passed / 16 skipped; 13 of 13 mutations caught); live validation INCOMPLETE: the Tier 1 fixture scored 35 but the model skipped the pause (the <80 gate is not enforced in Python; errors.md 2026-09-22). Status: code-complete, live-validation-blocked by Build F2 (below). No further F1 live calls until F2 is fixed and evaluated; then re-run Tier 1 and Tier 2 (correct path). See decisions.md 2026-09-22 (Build F1).
- [ ] Build F2 (HIGHER PRIORITY — blocks F1's live validation): enforce the <80 domain-confidence gate in Python. Today the gate exists only in profiler_system.md; profiler_node checks only the response's `type`, so a sub-80 ProfileReport proceeds on ANY run with no pause and no error. Direct evidence: LangSmith run 1d5b47c9-4fb9-4866-b579-3dc0ea1a2a5b (score 35, full ProfileReport, domain "unknown"). Pre-existing; never surfaced because every earlier live run scored 97+. Open design question the evaluation must resolve explicitly, not assume: what a domain-pause answer (and `domain_resolution`) should look like when the model's own hypothesis is "unknown" / genuinely unresolvable — possibly a third response shape distinct from confirm/correct rather than absorbed into either. Needs its own evaluation with F1's rigor. See errors.md 2026-09-22.
- [ ] Build F3: the Cleaner's multi-column pause loop + the missing 4th missing-value option (separate evaluation; cleaner.py / cleaner_system.md)
