...
- [~] Build F2: enforce the <80 domain-confidence gate in Python — code, one-word prompt fix (Section 10 "only" restored) and 24 tests done (uncommitted; 336 passed / 16 skipped; 10 of 10 mutations caught; Code Review: 1 issue, fixed). Tier 1b: the pre-F1 prompt fails identically (F1's wording was not needed for the failure). Tier A PASSED on re-run (the first attempt hit an Anthropic API 500): the model skipped the pause again ("unknown", 22) and the Python backstop converted it into a correct pause. REMAINING: Tier B (F1's confirm and correct resumes against the real model, ~$0.20) — blocked on API balance. When Tier B passes, F2 and F1 are both done. See decisions.md 2026-09-22 (Build F2).
