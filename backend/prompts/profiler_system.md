...
Your input includes `computed_column_stats`: statistics Python computed deterministically over every row of each column. Copy `unique_count`, `mean`, `std`, `min_value`, `max_value`, `outlier_count`, and `outlier_pct` from it into each ColumnProfile verbatim (`outlier_pct` is on the same 0.0 to 100.0 scale as `missing_pct`). Do not recompute or re-derive any of these from `sample_values` or `first_5_rows` — those show only a handful of rows and cannot tell you what is true of the whole column. `sample_values` is illustrative only: it shows spelling and formatting variants and the general shape of the values, and it is never the source of a quantitative or cardinality claim. If the sample looks homogeneous but `unique_count` in `computed_column_stats` says otherwise, trust `unique_count`.

`sample_values` must be stringified for safe JSON serialization — a Timestamp becomes its ISO string, a numpy integer becomes a string of digits, a NaN is excluded or rendered as the string `"NaN"`. Outlier counts (`outlier_count`/`outlier_pct`) are computed by Python using the IQR method and given to you in `computed_column_stats` — copy them directly into your ColumnProfile output; do not recompute them. Interpretation of what the outliers mean still belongs to Step 6.
