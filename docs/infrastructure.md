...
**Step 11:** Check if any column has over 30% missing values OR if any domain-sensitive outlier decision is needed (medical data with extreme values, financial data with potential fraud signals). If yes: enter PAUSE state, one question per run. Present the specific decisions needed to the user with the options available for each. Wait for user responses. If no: proceed directly to Step 12. The Cleaner's LLM asks the question, but Python checks it before it is stored or shown (cleaner.py `validate_cleaner_pause`): the exact option ids, an executable imputation method, counts recomputed by Python, a fourth missing-value option (`preserve_missingness`) written by Python, and a loud error rather than a loop if an answered (pause type, column) pair is asked again. If the LLM returns a full report while a column is still over 30% missing and unanswered, Python turns it into that column's pause (`apply_missingness_backstop`); if the report routes a column's outliers medical or financial in its `outlier_review` but emitted no pause, Python turns it into that outlier pause (`apply_outlier_backstop`, Build F3b). Every answer accumulates in pipeline state (`answered_cleaner_pauses`) and is sent to each later Cleaner run.

**Step 12:** Thoughtful Cleaner (cleaner.py) executes with all user inputs incorporated. The LLM decides and explains; Python executes and records. Every LLM decision names one operation from a closed set (`convert_type`, `standardize_values`, `fill_missing`, `leave_missing`, `flag_outliers`, `note`) that Python validates and runs; nothing is executed from a decision's wording, and a decision with no valid operation is recorded as not executed. The order is fixed: Python removes exact duplicate rows itself (a system step with its own count), then the LLM's conversions, value standardizations and fills, then the user's pause choices (executed by option id in `apply_user_decisions`, attributed to the user), then the LLM's outlier flags. On a column the user decided, only an LLM `note`, a `flag_outliers` where the user answered only the missing-value pause, or a `fill_missing` / `leave_missing` where the user answered only the outlier pause, is kept. Every decision record is written by Python from what ran, with Python's counts; each operation checks its own effect, and failed checks go to `re_profile_verification.discrepancies`. `cleaning_report.operations` logs every operation with its status and facts (Build G).
